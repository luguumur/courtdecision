...
public class OccupationTypeSelectionModel extends AbstractSelectModel implements
		Serializable {
	...
	private Comparator<OccupationType> empComparator = new Comparator<OccupationType>() {

		public int compare(OccupationType o1, OccupationType o2) {
	...
	public OccupationTypeSelectionModel(CoreDAO cdao) {
		List<OccupationType> list = cdao.getListOccupationType();
		setLists(list,true);
	}
	
	private void setLists(List<OccupationType> list, boolean preSurname) {

		Collections.sort(list, empComparator);

		for (int i = 0; i < list.size(); i++) {
			
			OccupationType value = (OccupationType) list.get(i);
