/* 
 * File Name: 		EmployeeOrgJudgeSelectionModel.java
 * 
...
public class EmployeeOrgJudgeSelectionModel extends AbstractSelectModel implements
		Serializable {
	...
	public EmployeeOrgJudgeSelectionModel(CoreDAO cdao, Organization org, AppointmentLevel appointmentLevel) {
		List<Employee> list = cdao.getListEmployeeJudje(org, appointmentLevel);
	...
	private void setLists(List<Employee> list, boolean preSurname) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			options.add((preSurname)? new OptionModelImpl(value.getFullNameFirstChar(), value) :  new OptionModelImpl(value.getId().toString(), value));
		}
	}
