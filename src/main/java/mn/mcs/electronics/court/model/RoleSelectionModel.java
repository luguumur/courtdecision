...
public class RoleSelectionModel extends AbstractSelectModel implements
		Serializable {
	...
	private Comparator<Role> empComparator = new Comparator<Role>() {
	...
	public RoleSelectionModel(CoreDAO cdao) {
		List<Role> list = cdao.getRoleList();

		setLists(list,true);
	}
	
	private void setLists(List<Role> list, boolean preSurname) {

		Collections.sort(list, empComparator);

		for (int i = 0; i < list.size(); i++) {
			
			Role value = (Role) list.get(i);
