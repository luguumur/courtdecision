...
public class SchoolSelectionModel extends AbstractSelectModel implements
		Serializable {
	...
	private Comparator<School> empComparator = new Comparator<School>() {
	...
	public SchoolSelectionModel(CoreDAO cdao, Country country,
			UniversityType type) {
		List<School> list = cdao.getListSchool(type);

		setLists(list, true);
	}

	private void setLists(List<School> list, boolean preSurname) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			School value = (School) list.get(i);
