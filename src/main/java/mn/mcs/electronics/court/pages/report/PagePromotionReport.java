...
public class PagePromotionReport {
	...
	@Property
	@Persist
	private List<Object[]> listPromotion;
	
	@Property
	@Persist
	private Object[] valuePromotion;
	...
	void beginRender(){
		listPromotion= dao.getListPromotionReport(org,year);
	...
	public String getGeneralJudgeAuthorityName(){
		if(valuePromotion==null)
			return "";
		return valuePromotion[1].toString();
	...
	public String getCountEdu(){
		...
		return valuePromotion[0].toString();
	}
	...
	void onActionFromExport() {
		...
		try {
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 2, messages
					.get("employeePromotionReport"), styles.get("title"), 3);
			...
			if (listPromotion != null)
				for (Object[] s : listPromotion) {

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (listPromotion.indexOf(s)+1)+"", styles
									.get("plain-left-wrap-border"));
		
					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++,messages.get("promotion"), styles
									.get("plain-left-wrap-border"));
