...
public class CompEmployeeJobHistory {
	...
	// @InjectPage
	// private CompEmployeeMovement compEmployeeMovement;
	 ...
	 * 
	 * compEmployeeMovement.setEmployee(employee);
	 * compEmployeeMovement.setCurrentOccupation(currentOccupation);
	 * compEmployeeMovement
	 * .setDisplacementTypeSelect(DisplacementType.SHILJIH); return
	 * compEmployeeMovement;
	 * 
	 * return null; }
	 */
	Object onActionFromEditQuitJob(QuitJob quitJob) {
		...
		/*
		 * compEmployeeMovement.setEmployee(employee);
		 * compEmployeeMovement.setQuiJob(quitJob);
		 * compEmployeeMovement.setDisplacementTypeSelect
		 * (DisplacementType.AJLAASGARAH); return compEmployeeMovement;
		 */
		return null;
	}
	...
	public String getQuitDatea() {
		...
		if (valueCurrentOccupation == null
				|| valueCurrentOccupation.getQuitDate() == null)
			return "";
	...
	public String getTuAaLevel() {
		if (valueCurrentOccupation.getTuAaLevel() == null
				|| valueCurrentOccupation == null)
			return "";

		return valueCurrentOccupation.getTuAaLevel().getTuAaSort().getName()
				+ "-"
				+ valueCurrentOccupation.getTuAaLevel().getTuAaRank().getRank();
	}

	public String getAppointmentName() {
		if (valueCurrentOccupation == null
				|| valueCurrentOccupation.getAppointment() == null)
			return "";
	...
	public String getOrganizationName() {
		if (valueCurrentOccupation == null
				|| valueCurrentOccupation.getOrganization() == null)
			return "";
	...
	void onActionFromExportEmpQuitJob() {
		...
		try {
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 2,
					messages.get("employeeQuitJob"), styles.get("title"), 5);
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 2,
					messages.get("type-label"), styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 3,
					messages.get("quitCause-label"), styles.get("header-wrap"));
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 5,
					messages.get("money-label"), styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 6,
					messages.get("quitDate-label"), styles.get("header-wrap"));
			...
			if (listQuitJob != null)
				for (QuitJob s : listQuitJob) {
					...
					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (listQuitJob.indexOf(s) + 1) + "",
					...
					ExcelAPI.setCellValue(
							...
							rowIndex,
							colIndex++,
							(s.getQuitType() != null) ? messages.get(s
									.getQuitType().toString()) : "", styles
									.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (s.getQuitCause() != null) ? s
									.getQuitCause().getCauseName() : "", styles
									.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (s.getMonth() != null) ? s.getMonth()
									.toString() : "", styles
									.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (s.getMoney() != null) ? s.getMoney()
									.toString() : "", styles
									.get("plain-left-wrap-border"));
					...
					ExcelAPI.setCellValue(
							...
							rowIndex,
							colIndex++,
							(s.getQuitDate() != null) ? format.format(s
									.getQuitDate()) : "", styles
									.get("plain-left-wrap-border"));
	...
	void onActionFromexportEmpMovement() {
		...
		try {
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 2,
					messages.get("employeeMovement"), styles.get("title"), 9);
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 1,
					messages.get("number-label"), styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 2,
					messages.get("appointment-label"),
					styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 3,
					messages.get("toAppointment-label"),
					styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 4,
					messages.get("tuAaLevel-label"), styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 5,
					messages.get("department-label"), styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 6,
					messages.get("toDepartment-label"),
					styles.get("header-wrap"));
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 8,
					messages.get("toOrganization-label"),
					styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 9,
					messages.get("IssuedDatea-label"),
					styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 10,
					messages.get("QuitDatea-label"), styles.get("header-wrap"));
			...
			if (listCurrentOccupation != null)
				for (Displacement s : listCurrentOccupation) {
					...
					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (listCurrentOccupation.indexOf(s) + 1)
									+ "", styles.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++,
							(s.getAppointment() != null) ? s.getAppointment()
									.getAppointmentName() : "", styles
									.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (s.getTuAaLevel() != null) ? s
									.getTuAaLevel().getTuAaSort().getName()
									+ "-"
									+ s.getTuAaLevel().getTuAaRank().getRank()
											.toString() : "",
							styles.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (s.getOrganization() != null) ? s
									.getEmployee().getOrganization().getName()
									: "", styles.get("plain-left-wrap-border"));
					...
					ExcelAPI.setCellValue(
							...
							rowIndex,
							colIndex++,
							(s.getIssuedDate() != null) ? format.format(s
									.getIssuedDate()) : "", styles
									.get("plain-left-wrap-border"));
	...
	void onActionFromexportEmpExpierence() {
		...
		try {
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 2,
					messages.get("employeeExpierence"), styles.get("title"), 5);
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 1,
					messages.get("number-label"), styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 2,
					messages.get("organizationtype-label"),
					styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 3,
			...
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 4,
					...
					styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 5,
					messages.get("issuedDate-label"), styles.get("header-wrap"));
			ExcelAPI.setCellValue(document, sheetNumber, rowIndex, 6,
					messages.get("quitDate-label"), styles.get("header-wrap"));
			...
			if (listExperience != null)
				for (Experience s : listExperience) {
					...
					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (listExperience.indexOf(s) + 1) + "",
							styles.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++,
							messages.get(s.getOrganizationtype().toString()),
							styles.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, s.getOrganizationname().toString(),
							styles.get("plain-left-wrap-border"));

					ExcelAPI.setCellValue(document, sheetNumber, rowIndex,
							colIndex++, (s.getAppointment() != null) ? s
									.getAppointment().toString() : "", styles
									.get("plain-left-wrap-border"));
